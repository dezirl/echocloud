...
export const useStore = create<EchoCloudState>((set, get) => ({
  ...
  playTrack: async (trackId) => {
    ...
    set((state) => {
      ...
      return { currentTrackId: trackId, isPlaying: true, currentTime: 0, queue: updatedQueue };
    });
  },
  ...
  nextTrack: () => {
    const { contextQueue, manualQueueNext, currentTrackId, isShuffle, isRepeat, playTrack } = get();
    ...
    // Navigate through context queue (current view), fall back to legacy queue
    const src = contextQueue.length > 0 ? contextQueue : get().queue;
  ...
  prevTrack: () => {
    const { contextQueue, currentTrackId, isShuffle, playTrack, currentTime } = get();
    ...
    for (const t of incoming) {
      ...
      if (existing) {
        byId.set(t.id, {
          ...
          // Prefer whichever has a transcoding URL (incoming might have it if existing was a stub)
          scTranscodingUrl: existing.scTranscodingUrl || (t as any).scTranscodingUrl || '',
          // Keep resolved streamUrl unless empty
  ...
  setHomeSections: (sections) => set((state) => {
    ...
    for (const sec of sections) {
      for (const t of sec.tracks) {
        ...
        if (!existing) {
          byId.set(t.id, t);
        } else if (!(existing as any).scTranscodingUrl && (t as any).scTranscodingUrl) {
          // Upgrade stub track with transcoding URL from home sections
          byId.set(t.id, { ...existing, scTranscodingUrl: (t as any).scTranscodingUrl } as any);
        }
      }
    }
