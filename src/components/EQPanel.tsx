...
function VerticalSlider({
  ...
  return (
    <div
      ...
      className={`relative select-none shrink-0 ${disabled ? 'opacity-25 pointer-events-none' : 'cursor-ns-resize'}`}
    ...
    <div className="fixed bottom-[92px] right-4 w-[560px] max-w-[calc(100vw-32px)] bg-neutral-950/98 backdrop-blur-3xl border border-white/[0.07] rounded-2xl z-50 shadow-[0_-8px_40px_rgba(0,0,0,0.8)] overflow-hidden">
      ...
      <div className="flex items-center justify-between px-5 pt-4 pb-3">
        <div className="flex items-center gap-3">
          ...
          <button
            onClick={toggleEq}
            className="px-2.5 py-0.5 text-[9px] font-mono font-bold rounded-md border cursor-pointer transition-all"
            style={eqEnabled ? {
              backgroundColor: `${accent}18`, borderColor: `${accent}45`, color: accent,
            } : {
              backgroundColor: 'rgba(255,255,255,0.04)', borderColor: 'rgba(255,255,255,0.06)', color: '#52525b',
            }}
          >
            {eqEnabled ? 'ON' : 'OFF'}
          </button>
        ...
        <div className="flex items-center gap-3">
          {Object.keys(PRESETS).map(name => (
            <button
              key={name}
              onClick={() => { setEqBands(PRESETS[name].bands); setEqQ(PRESETS[name].q); }}
              disabled={!eqEnabled}
              className="text-[8.5px] font-mono text-zinc-500 hover:text-zinc-200 px-2 py-0.5 rounded border border-white/[0.05] hover:border-white/[0.12] cursor-pointer transition-colors disabled:opacity-30 disabled:cursor-not-allowed"
            >
              {name}
            </button>
          ))}
          <button
            ...
            className="text-[8.5px] font-mono text-zinc-500 hover:text-zinc-300 cursor-pointer transition-colors tracking-widest"
          >
            RESET
          </button>
          <button
