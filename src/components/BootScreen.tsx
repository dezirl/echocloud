...
export default function BootScreen({ triggerNotification }: BootScreenProps) {
  ...
  useEffect(() => {
    ...
    const cleanup = window.electronAPI.onAutoLogin((user) => {
      loginSession(user.username, 'auto');
      updateUserProfile({ ...user, scUserId: (user as any).scUserId });
      triggerNotification('Welcome back', `Signed in as @${user.username}`);
    });

    // Give the main process 8s to send auto-login before showing the form
    const timeout = setTimeout(() => setCheckingSession(false), 8000);
                ...
                <AnimatePresence mode="wait">
                  {!browserOpened ? (
                    ...
                    >
                      ...
                      <p className="text-[9px] text-zinc-700 text-center leading-relaxed px-1">
                        Tip: In Firefox use DevTools → Storage → Cookies → soundcloud.com
                      </p>
                    </motion.div>
                  )}
                </AnimatePresence>
