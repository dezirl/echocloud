...
import { Minimize2, Play, Pause, SkipBack, SkipForward, Volume2, VolumeX, Shuffle, Repeat, Heart, Disc, Sparkles, Music, Settings, RotateCcw } from 'lucide-react';
import { useStore } from '../store';
import { HolographicBackground, type VizSettings } from './HolographicBackground';
import { THEME_ACCENT } from './EQPanel';

const VIZ_DEFAULTS: VizSettings = { intensity: 1.0, speed: 1.0 };
...
export default function FullscreenPlayer({ isOpen, onClose }: FullscreenPlayerProps) {
  ...
  const updateViz = (key: keyof VizSettings, value: number) => {
    vizSettingsRef.current = { ...vizSettingsRef.current, [key]: value };
    setVizSettings(prev => ({ ...prev, [key]: value }));
  };
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      if (alb) {
        ...
        if (playing) {
          alb.style.transform = `scale(${1 + beatPeak * 0.22 * intensity})`;
          const shadowR = 30 + beatPeak * 90 * intensity;
          alb.style.boxShadow = `0px 20px ${shadowR}px rgba(${ac}, ${0.08 + bassValue * 0.55})`;
        } else {
          alb.style.transform = 'scale(1)';
          alb.style.boxShadow = `0px 20px 30px rgba(${ac}, ${0.06 * fadeMultiplier})`;
        }
      }
              ...
              {vizSettingsOpen && (
                <div className="absolute top-12 right-0 z-50 w-72 bg-neutral-950/97 border border-white/[0.08] rounded-2xl p-4 backdrop-blur-3xl shadow-[0_8px_40px_rgba(0,0,0,0.7)]">
                  ...
                  <div className="mt-3 pt-3 border-t border-white/[0.05]">
                    <div className="h-px w-full" style={{ background: `linear-gradient(to right, transparent, ${accentColor}40, transparent)` }} />
                  </div>
                </div>
              )}
            </div>
        ...
        <div className="px-8 lg:px-24 py-10 relative z-20 border-t border-white/[0.03] bg-neutral-950/45 backdrop-blur-3xl flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 items-center mt-1">
            ...
            <div className="flex items-center gap-3 justify-end">
              ...
              <div className="relative w-16 md:w-24 h-5 flex items-center cursor-pointer">
                <div className="absolute inset-x-0 h-[3px] rounded-full overflow-hidden pointer-events-none" style={{ backgroundColor: 'rgba(255,255,255,0.15)' }}>
