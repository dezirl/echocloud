...
function normalizeTrack(sc: any) {
  return {
    ...
    likesCount: sc.likes_count ?? sc.favoritings_count ?? 0,
...
export class SoundCloudService {
  ...
  async getFollowers(userId: number): Promise<{ users: any[]; nextHref: string | null }> {
    try {
      const res = await this._get(`/users/${userId}/followers`, { limit: '50', linked_partitioning: '1' });
      const users = (res.collection ?? []).map((u: any) => ({
        id: u.id as number,
        username: u.username || 'Unknown',
        displayName: u.full_name || u.username || 'Unknown',
        avatarUrl: (u.avatar_url || '').replace('-large', '-t200x200'),
      ...
      return { users, nextHref: res.next_href ?? null };
    } catch (e) {
      console.error('[SC followers]', e);
      return { users: [], nextHref: null };
    }
  }

  async loadMoreFollowers(nextHref: string): Promise<{ users: any[]; nextHref: string | null }> {
    try {
      const res = await this._get(nextHref);
      const users = (res.collection ?? []).map((u: any) => ({
        id: u.id as number,
      ...
      console.error('[SC loadMoreFollowers]', e);
      return { users: [], nextHref: null };
    }
  }
