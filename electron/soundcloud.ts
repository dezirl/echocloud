...
/** Pick the best progressive MP3 transcoding URL from a track's media object. */
function pickTranscodingUrl(sc: any): string {
  const transcodings: any[] = sc?.media?.transcodings ?? [];
  const best =
    transcodings.find((t) => t.format?.protocol === 'progressive' && t.format?.mime_type === 'audio/mpeg') ||
  ...
  async getHomeSections(): Promise<Array<{ id: string; title: string; tracks: any[]; nextHref?: string }>> {
    ...
    if (this.authToken) {
      ...
      try {
        ...
        const tracks = (recent.collection ?? []).map(normalizeTrack).slice(0, 12);
